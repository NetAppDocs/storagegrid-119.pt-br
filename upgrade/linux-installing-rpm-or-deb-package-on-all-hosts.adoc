...
summary: Se algum nó StorageGRID estiver implantado em hosts Linux, instale um pacote RPM ou DEB adicional em cada um desses hosts antes de iniciar a atualização. 
---
...
[role="lead"]
Se algum nó StorageGRID estiver implantado em hosts Linux, baixe e instale um pacote RPM ou DEB adicional em cada um desses hosts antes de iniciar a atualização.



== Baixar arquivos de atualização, Linux e hotfix

Ao executar uma atualização do StorageGRID no Grid Manager, você será solicitado a baixar o arquivo de atualização e qualquer hotfix necessário como primeira etapa.  No entanto, se você precisar baixar arquivos para atualizar hosts Linux, poderá economizar tempo baixando todos os arquivos necessários com antecedência.
...
. Se um aviso de Cuidado/Leitura Obrigatória aparecer, anote o número do hotfix e marque a caixa de seleção.
. Leia o Contrato de Licença do Usuário Final (CLUF), marque a caixa de seleção e selecione *Aceitar e Continuar*.
+
A página de downloads da versão selecionada será exibida.  A página contém três colunas.

. Na segunda coluna (*Upgrade StorageGRID*), baixe dois arquivos:
+
** O arquivo de atualização para a versão mais recente (este é o arquivo na seção denominada *VMware, SG1000 ou SG100 Primary Admin Node*).  Embora esse arquivo não seja necessário até que você execute a atualização, baixá-lo agora economizará tempo.
** Um arquivo RPM ou DEB em qualquer um `.tgz` ou `.zip` formatar.  Selecione o `.zip` arquivo se você estiver executando o Windows no laptop de serviço.
+
...
. Se você precisou concordar com um aviso de Cuidado/Leitura Obrigatória devido a um hotfix necessário, baixe o hotfix:
+
.. Voltar para https://mysupport.netapp.com/site/products/all/details/storagegrid/downloads-tab["Downloads da NetApp : StorageGRID"^] .
.. Selecione o número do hotfix no menu suspenso.
.. Concorde novamente com o aviso de Cuidado e o CLUF.
.. Baixe e salve o hotfix e seu README.
+
Você será solicitado a carregar o arquivo de hotfix na página de atualização do StorageGRID quando iniciar a atualização.
...
== Instalar arquivo em todos os hosts Linux
...
+
Veja as etapas para instalar os serviços do host StorageGRID nas instruções de instalação:

+
** link:../rhel/installing-storagegrid-webscale-host-service.html["Red Hat Enterprise Linux: Instalar serviços de host StorageGRID"]
** link:../ubuntu/installing-storagegrid-webscale-host-services.html["Ubuntu ou Debian: instalar serviços de host StorageGRID"]
...
== Remover arquivos de instalação de versões anteriores

Para liberar espaço em hosts Linux, você pode remover os arquivos de instalação de versões anteriores do StorageGRID que não são mais necessárias.
...
CAUTION: Não remova os arquivos de instalação da versão do StorageGRID que você está executando no momento ou das versões do StorageGRID para as quais você planeja atualizar.
...
--
====
. Remover imagens de contêiner StorageGRID .
...
.Docker
--
. Capture a lista de imagens de contêiner instaladas: `docker images`
+
Exemplo:
...
[root@docker-example ~]#
----
. Remova as imagens de contêiner das versões anteriores do StorageGRID : `docker rmi _image id_`
+

CAUTION: Não remova as imagens de contêiner da versão do StorageGRID que você está executando no momento ou das versões do StorageGRID para as quais você planeja atualizar.
...
.Podman
--
. Capture a lista de imagens de contêiner instaladas: `podman images`
+
Exemplo:
...
[root@podman-example ~]#
----
. Remova as imagens de contêiner das versões anteriores do StorageGRID : `podman rmi _image id_`
+
