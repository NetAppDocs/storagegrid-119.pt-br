...
summary: Quando um cliente S3 emite uma transação HEAD, uma solicitação é feita para verificar a existência de um objeto ou bucket e recuperar os metadados sobre um objeto.  Esta mensagem é emitida pelo servidor se a transação for bem-sucedida. 
---
= SHEA: CABEÇA S3
:allow-uri-read: 
:icons: font
...
Tamanho do conteúdo
 a| 
O tamanho do objeto verificado em bytes.  Operações em buckets não incluem este campo.
